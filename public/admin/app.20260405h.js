...
const SERVER_UPLOAD_CONCURRENCY = 3;
...
async function selectLive(liveId) {
    ...
    const [albumsResponse, photosResponse, logsResponse, analyticsResponse] = await Promise.all([
        api(`/albums/live/${liveId}`),
        api(`/photos/admin/live/${liveId}`),
        api(`/logs/live/${liveId}?limit=50`),
        api(`/analytics/live/${liveId}`)
    ]);

    state.albums = albumsResponse.data;
    state.photos = photosResponse.data;
...
function fillLiveForm(live) {
    ...
    elements.showBannerInput.value = Number(live.show_banner ?? 0);
...
function renderBannerImageSettings() {
    ...
    elements.uploadBannerImageButton.textContent = state.bannerDraftFile ? '上传并设为头图' : '头图已同步';
        ...
        await runLimitedConcurrency(chunk, DIRECT_UPLOAD_CONCURRENCY, async (item, offset) => {
            ...
            if (target.reused) {
                ...
                updateUploadItem(index, {
                    ...
                    progress: 100,
                    status: 'done',
                    statusText: result.message || target.message || '云端复用完成'
                });
                state.uploadSession.registeredCount += 1;
                incrementUploadCompleted();
                return;
            ...
            updateUploadItem(index, {
                progress: 100,
                status: 'done',
                statusText: result.message || '上传完成'
            });
            state.uploadSession.registeredCount += 1;
            incrementUploadCompleted();
        });
...
function renderPhotoCount() {
    ...
    elements.photoCountText.textContent = state.batchMode
        ? `已选 ${selectedCount} 张 · 当前 ${filtered.length} / ${state.photos.length} 张`
        : `${filtered.length} / ${state.photos.length} 张照片`;
}
...
function renderPhotoInspector() {
    ...
    const deviceText = exif.camera || exif.Model || exif.Make || '未读取到 EXIF';
    const settingsText = [exif.aperture, exif.shutterSpeed, exif.iso ? `ISO ${exif.iso}` : ''].filter(Boolean).join(' / ') || '未读取到拍摄参数';
    const metaItems = [
...
function getFilteredPhotos() {
    ...
    const items = state.photos.filter((photo) => {
        if (state.filters.albumId !== 'all' && String(photo.album_id || '') !== state.filters.albumId) {
            return false;
        }
        if (state.filters.visibility === 'public' && Number(photo.is_public) !== 1) {
            return false;
        }
        if (state.filters.visibility === 'private' && Number(photo.is_public) !== 0) {
            return false;
        }
        if (keyword) {
            const haystack = [photo.title, photo.description, photo.album_name, ...(photo.tags || [])].join(' ').toLowerCase();
            if (!haystack.includes(keyword)) {
                return false;
            }
        }
        if (state.filters.camera) {
            const value = String(photo.exif_data?.camera || photo.exif_data?.Model || photo.exif_data?.Make || photo.camera_search || '').trim();
            if (value !== state.filters.camera) {
                return false;
            }
        }
        if (state.filters.lens) {
            const value = String(photo.exif_data?.lens || photo.exif_data?.LensModel || photo.lens_search || '').trim();
            if (value !== state.filters.lens) {
                return false;
            }
        }
        if (state.filters.focalLength) {
            const value = String(photo.exif_data?.focalLength || photo.focal_length_search || '').trim();
            if (value !== state.filters.focalLength) {
                return false;
            }
        }
        if (state.filters.aperture) {
            const value = String(photo.exif_data?.aperture || photo.aperture_search || '').trim();
            if (value !== state.filters.aperture) {
                return false;
            }
        }
        if (state.filters.iso) {
            const value = String(photo.exif_data?.iso || photo.iso_value || '').trim();
            if (value !== state.filters.iso) {
                return false;
            }
        }
        if (state.filters.format) {
            const value = String(photo.exif_data?.format || photo.format_value || '').trim().toUpperCase();
            if (value !== state.filters.format) {
                return false;
            }
        }
        return true;
    ...
    items.sort((left, right) => {
        ...
        if (state.filters.sort === 'popular') {
            return Number(right.like_count || 0) - Number(left.like_count || 0) || new Date(right.created_at) - new Date(left.created_at);
        }
        if (state.filters.sort === 'views') {
            return Number(right.view_count || 0) - Number(left.view_count || 0) || new Date(right.created_at) - new Date(left.created_at);
        }
...
function getSelectedPhoto() {
    return state.photos.find((photo) => photo.id === Number(state.selectedPhotoId)) || null;
}
...
async function batchUpdateAlbum() {
    ...
    for (const photo of photos) {
        await api(`/photos/${photo.id}`, {
            method: 'PUT',
            body: JSON.stringify(buildPhotoUpdatePayload(photo, { album_id: albumId }))
        });
    }
...
async function batchAppendTags() {
    ...
    for (const photo of photos) {
        const mergedTags = Array.from(new Set([...(photo.tags || []), ...extraTags]));
        await api(`/photos/${photo.id}`, {
            method: 'PUT',
            body: JSON.stringify(buildPhotoUpdatePayload(photo, { tags: mergedTags }))
        });
    }
...
async function batchDeleteSelectedPhotos() {
    ...
    await runDeleteTask({
        title: '批量删除照片',
        total: photos.length,
        summary: `准备删除 ${photos.length} 张照片`,
        worker: async ({ step, advance }) => {
            step('正在提交批量删除', `正在删除 ${photos.length} 张照片`);
            await api('/photos/batch-delete', {
                method: 'POST',
                body: JSON.stringify({
